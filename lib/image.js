...
Image.create = function (image, fn) {
...
Image.prototype = {
  ...
  saveOriginal: function (original, fn) {
    ...
    Identity.fromImageStream(original, function (err, identity) {
      ...
      db.query('INSERT INTO images (id, mime_type, size, width, height) VALUES (?,?,?,?,?)', [self.id, identity.mimeType(), identity.size(), identity.width(), identity.width()], function (err, results) {
        fn(err, identity)
      })
    })
  },
